\section{Preliminaries}
	...
	\begin{definition}
		For any poset $(P, \le)$, let $\sigma$ be a permutation of $P$. We define the $inversions$ of $\sigma$ to be a binary relation $\Inv_{\sigma}$ on $P$:
		\[
			\Inv_{\sigma} = \{(i,j) \mid i, j \in P, i < j, \sigma^{-1}(i) > \sigma^{-1}(j)\}
		\]
		$\Inv$ is a transitive relation because for any $a,b,c \in P$ if $a \Inv b$ and $b \Inv c$ then $a < b < c$ and $\sigma^{-1}(a) > \sigma^{-1}(b) > \sigma^{-1}(c)$ which means that $a \Inv c$. In addition, for any $a,b \in P$ we have
		\[
			\Inv_{a \meet b} = (\Inv_a \cup \Inv_b)^t
		\]
	...
	\begin{definition}[$\le_s$]
		Let $(P, \le)$ be a poset. We define the partial ordering $\le_s$ on $P$ such that for all $x, y \in P$:
		\[
			x \le_s y \iff \Inv_y \subseteq \Inv_x
		\]
	\end{definition}
