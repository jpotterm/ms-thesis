...
\section{Generalized Lemma 8 of Friedgut}
	...
	\begin{proof}
		We define $A'$ to be a consolidation of $A$ as follows.
			...
			\begin{algorithmic}
				\For{$i = 1 \to n$}
					\For{$d \in L(C)^n$}
						...
						\For{$a \in A|_D$}
							\State $x \gets \max(A|_D \backslash A')$
		...
		We do the same to consolidate $B$ to $B'$.

		For each $D$ above, we now have that either $A'|_D \subseteq B'|_D$ or $B'|_D \subseteq A'|_D$. Unfortunately, we need $A' \subseteq B'$ or $B' \subseteq A'$ which does not necessarily follow; see Open Problem \ref{lemma-8-open-problem}.
		...
		\begin{openproblem}
			...
			\[
				x \gets \max(A|_D \backslash A')
			\]
			to something like
			\[
				x \gets \max(A \backslash A')
			\]
