...
\section{Lattice Theory}
	...
	\begin{proof}
		...
		\[
			x \le_s z \textrm{ and } y \le_s z.
		\]
		And $z$ is the least upper bound of $\{x, y\}$. For every $a \in X$:
		\[
			(x \le_s a \textrm{ and } y \le_s a) \implies z \le_s a.
		\]
		Since $x \in X^{ij}$, then $(i, j) \notin Inv_x$. Since $x \le_s z$, then $(i, j) \notin Inv_z$, so $z \in X^{ij}$. By definition $x \le_s z \implies x \le^{ij}_s z$ and $y \le_s z \implies y \le^{ij}_s z$. Therefore $z$ is an upper bound of $\{x, y\}$ in $X^{ij}$.
	...
	\begin{proof}
		...
		For any $a \in X^{ij}$ if $a$ is a lower bound of $\{x, y\}$ in $X^{ij}$ then clearly $a$ is also a lower bound of $\{x, y\}$ in $X$. Therefore $a \le_s z$, so $a \le^{ij}_s z$, which means $z = x \meet^{ij} y$. So clearly $x \meet^{ij} y$ is defined.
	\end{proof}
	...
	\begin{proof}
		...
		\[
			s_i \le u_i \textrm{ and } s_i \le u_i
		\]
		so
		\[
			s \le^n u \textrm{ and } t \le^n u
		\]
		...
		\[
			s_i \le v_i \textrm{ and } t_i \le v_i
		\]
...
\section{Generalized Lemma 6 of Friedgut}
	...
	\begin{proof}
		...
		\]
		is the probability of a profile that satisfies $p|_{\{a,b\}} = p'|_{\{a,b\}}$ also satisfies $f(p) = a$. And
		\[
			\frac{|B(p)|}{\left(\frac{m!}{2}\right)^n}
		\]
		is the probability of a profile that satisfies $p|_{\{a,b\}} = p'|_{\{a,b\}}$ also satisfies $f(p) = b$.
	\end{proof}
...
\section{Generalized Lemma 7 of Friedgut}
	...
	\begin{definition}
		Let $p \in L(C)^n$. We define the \emph{upper edge border} of $A(p)$, denoted $\partial A(p)$, to be the set of directed edges whose tail is in $A(p)$ and whose head is not in $A(p)$. Formally, for all $i \in \{1, \ldots, n\}$:
			\[
				\partial_i A(p) = \{ (x_{-i}, x_i, x'_i) \mid (x_{-i}, x_i) \in A(p), (x_{-i}, x'_i) \notin A(p), x_i|_{\{a,b\}} = x'_i|_{\{a,b\}}, x_i <_s x'_i \}
			\]
		and
			\[
				\partial A(p) = \bigcup_j \partial_j A(p)
			\]
		We define the upper edge border of $B(p)$ analogously. Note that while $(L(C), \le_s)$ is a lattice, we will treat the upper edge border as a graph.
	\end{definition}
	...
	\begin{proof}
		...
		\]
		and view $A(p), B(p)$, as residing in $P'$. This is because any element in $A(p)$ or $B(p)$, cannot possibly be in $L(C)^n \backslash P'$: by definition the elements of these sets agree with $p|_{\{a,b\}}$. Clearly $|P'| = (\frac{m!}{2})^n$.
